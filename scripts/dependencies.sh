...
case "$(uname -o)" in
    "Msys")
        ...
        ;;
    "MS/Windows")
        source "${PROGRAM_WORKING_DIRECTORY}/scripts/lib_w64devkit.sh"
        ;;
    "Linux")
        source "${PROGRAM_WORKING_DIRECTORY}/scripts/lib_linux.sh"
        ;;
    *)
        break
        ;;
...
main() {
    ...
    "${DEPENDENCY}_${SUBCOMMAND}" 1>/dev/null 2>&1 || printf "invalid dependency or subcommand: \"%s\" \"%s\"\n" "${DEPENDENCY}" "${SUBCOMMAND}"
}
...
raylib_all() {
    ...
    local HASH_REQUIRED="deaffb06981b95976f7b055dc1085adb933a37cc"

    if [ ! -d "${PATH_TARGET}" ]; then
        git clone "${HOST}" "${PATH_TARGET}" --depth=1 2>&1 1>&3 || exit 1
